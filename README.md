...
- `face_detection_model/` — Caffe model files for face detection
  - `deploy.prototxt`
  - `res10_300x300_ssd_iter_140000.caffemodel`
- `openface_nn4.small2.v1.t7` — OpenFace embedding model
- `output/` — saved models and encodings
  - `recognizer_tuned.pickle` — trained classifier
  - `le.pickle` — label encoder
  - `embeddings.pickle` — optional embeddings file
- `static/uploads/` — uploaded and result images
- `templates/` — HTML templates (`index.html`, `result.html`, `base.html`)

## Key Technology
---------------

- **Flask** — web framework
- **OpenCV** — image processing and DNN support
- **OpenFace (NN4.small2.v1)** — face embedding model (`openface_nn4.small2.v1.t7`)
- **Caffe SSD (ResNet-10)** — face detector (files in `face_detection_model/`)
- **scikit-learn (SVM)** — classifier saved in `output/recognizer_tuned.pickle`
